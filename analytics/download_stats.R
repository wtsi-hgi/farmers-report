...
# inputs
start_date <- Sys.Date() - 30*6
config_file <- "config-openstack.yaml"
outfolder <- "./data"

dir.create(outfolder, showWarnings = FALSE)

config <- read_config(config_file, section = "proxy")
...
while (date <= last_date) {
    ...
    df <- df %>%
        mutate(date = date) %>%
        group_by(date, BOM, ACCOUNTING_NAME, USER_NAME) %>%
        summarise(
            number_of_jobs = n(),
