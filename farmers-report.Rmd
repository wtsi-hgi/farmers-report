...
`r if(!params$static){"## Cluster usage"}`
```{r farm usage, echo=FALSE, fig.height=1.5}
b <- build_agg_query("CLUSTER_NAME")

res <- Search(elastic_con, index = index, body = b, asdf = T)

df <- parse_elastic_single_agg(res)

make_farm_job_count_plot(df)
```
...
df %>%
  mutate(
    mem_wasted_cost = mem_wasted_mb_sec * gb_ram_hour / 1024 / 60 / 60,
